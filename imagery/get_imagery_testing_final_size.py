...
def image_to_np(image,rectangle,region,directory):
    ...
    r = requests.get(image.getDownloadURL({'region': region, 'dimensions': [224,224] }))
...
#keeping NO2 only and ozone AQI
dataset=dataset[(dataset['type']=='NO2') | (dataset['type']=='OZONE_AQI')].reset_index(drop=True)
#keeping 100 examples of each class
dataset=dataset[1759:1959].reset_index(drop=True)
